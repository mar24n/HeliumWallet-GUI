...
function getStakeInputValue() {
    ...
    const confirm = (`Sending ${stake_amounthnt} HNT, To ${stake_address}`).replace(/,/g, '\n\n');
    alert(confirm);
    ...
    const staking_result = (`Validator: ${validator}, Hash: ${staking_hash}, Fee: ${transaction_fee}, Staking Fee: ${staking_fee}`).replace(/,/g, '\n\n');

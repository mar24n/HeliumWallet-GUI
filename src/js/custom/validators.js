  ...
  const address_n = localStorage.getItem("walletAddress");
    Promise.all([
       ...
       console.log(val_last_heartbeat);
        displayListVal(val_name, val_online, val_stake_status, val_penalty, val_last_heartbeat, val_version, block);
...
function displayListVal(val_name, val_online, val_stake_status, val_penalty, val_last_heartbeat, val_version, block) {
  ...
  document.getElementById("v_last_heartbeat").innerText = val_last_heartbeat + "/" + block;
